...
class Course(models.Model):
    ...
    order_details = GenericRelation("OrderDetail", related_query_name="api1")
    coupon = GenericRelation("Coupon")
    # 用于GenericForeignKey反向查询，不会生成表字段，切勿删除，如有疑问请联系老村长
...
class PricePolicy(models.Model):
    ...
    def __str__(self):
        return "%s(%s)%s" % (self.content_object,
                             self.get_valid_period_display(), self.price)
...
class Coupon(models.Model):
    ...
    coupon_valid_days = models.PositiveIntegerField(
        verbose_name="优惠券有效期（天）", blank=True, null=True, help_text="自券被领时开始算起")
...
class Order(models.Model):
    ...
    order_type = models.SmallIntegerField(
        choices=order_type_choices, default=0, verbose_name="订单类型")
...
class UserInfo(AbstractUser):
    ...
    id_card = models.CharField(
        ...
        blank=True,
        null=True,
        verbose_name="身份证号或护照号")
    password = models.CharField('password', max_length=128, help_text=mark_safe(
        '''<a class='btn-link' href='password'>重置密码</a>'''))
    is_active = models.BooleanField(default=True, verbose_name="账户状态")
    is_staff = models.BooleanField(
        verbose_name='staff status',
        default=False,
        help_text='决定着用户是否可登录管理后台')
    name = models.CharField(max_length=32, default="", verbose_name="真实姓名")
    head_img = models.CharField(
        max_length=256,
        default='/static/frontend/head_portrait/logo@2x.png',
        verbose_name="个人头像")
    role_choices = ((0, '学员'), (1, '导师'), (2, '讲师'),
                    (3, '管理员'), (4, '班主任'), (5, '线下班主任'))
