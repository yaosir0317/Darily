layui.define(['layer', 'form', 'tips'], function(exports) {
    ...
    $('.lau-sign-sms').click(function () {
        ...
        $.post('/user/code', {phone: phone, csrfmiddlewaretoken: csrfmiddlewaretoken}, function (json) {
            if (json.errcode == 0) {
    ...
    form.on('submit(register)', function (data) {
        ...
        // 将用户名赋值为手机号
        data.field['username'] = data.field.phone
        //发送注册表单
        $.post('/user/user_create', data.field, function (json) {
            if (json.errcode == 0) {
                tips.success(json.errmsg, function () {
                    location.href = '/user/account_login';
                });
                ...
                tips.error(json.errmsg.code[0], function () {
